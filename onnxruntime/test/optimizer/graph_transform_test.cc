...
TEST_F(GraphTransformationTests, GatherInternalNodeReplacement) {
  // Given
  // onnx graph generated by running the python test
  // orttraining_test_ortmodule_api.py::test_gradient_correctness_embedding
  // *_optimized_training.onnx
  auto model_uri = MODEL_FOLDER "gather_internal_node_replacement.onnx";
  std::shared_ptr<Model> model;
  ASSERT_STATUS_OK(Model::Load(model_uri, model, nullptr, *logger_));
  Graph& graph = model->MainGraph();
  auto op_to_count = CountOpsInGraph(graph);
  ASSERT_TRUE(op_to_count["com.microsoft.GatherInternal"] == 2);
